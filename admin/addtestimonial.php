<?php
    ...
    if (isset($_POST['submit']))
    {
        ...
        {
            $name = $testimonial->escapestring($_POST['name']);
        }
        else
        {
        ...
        if (count($error)==0)
        {
            ...
            $testimonial->created_by = $_SESSION['admin'];
            $testimonial->modified_by = $_SESSION['admin'];
            $date=date('Y-m-d H:i:s');
            $testimonial->created_at =$date;
            $testimonial->modified_at =$date;
            if($_FILES['image']['error']==0 && $_FILES['image']['size']!=0) 
            {
                $iname=$_FILES['image']['name'];
                move_uploaded_file($_FILES['image']['tmp_name'],'images/'.$iname);
                 $testimonial->image=$iname;
                $ask = $testimonial->inserttestimonial();
            }
